...
def preprocess(img, shave=False, shavedim=(350, 500, 500, 1000)):
    ...
    # resize the image that yolo requires
    resized = cv2.resize(border, (448, 448))

    # fed in by (channel, y,x). Transpose to match that.
...
# change yolo output into box values to the original image.
def process_output(yolo_output, threshold=0.2, padhw=(98, 0)
...
def draw_boxes(boxes, img):
    ...
    for box in boxes:
        ...
        img = cv2.putText(img, 'Car %{0:.3}'.format(box.prob * 100), (box.x1, box.y1 - 10), font, 1.6, (255, 255, 255),
                          2,
                          cv2.LINE_AA)
